...
def construct_binary_k_mers(k):
    k_mers = []
    for i in range(2**k):
        k_mers.append(bin(i)[2:].zfill(k))
    return k_mers
...
def main():
    k = int(input())
    k_mers = construct_binary_k_mers(k)
    graph = build_graph_from_k_mers(k_mers)
    cycle = find_eulerian_cycle_or_path(graph)
    if cycle is None:
        print("Graph is not eulerian.")
        return
    print(reconstruct_string_from_path(cycle[:-k+1]))
